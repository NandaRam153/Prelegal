...
// Verbatim Common Paper Mutual NDA Standard Terms v1.0 with coverpage_link markers intact.
...
export function getMndaTermText(v: MndaFormValues): string {
  if (v.mndaTermType === 'years') {
    const y = v.mndaTermYears;
    return `${y} year${y !== 1 ? 's' : ''} from Effective Date`;
  }
  return 'continuing until terminated';
}

export function getConfidentialityTermText(v: MndaFormValues): string {
  if (v.confidentialityTermType === 'years') {
    const y = v.confidentialityTermYears;
...
// Returns the cover page fields as label-value pairs for both HTML preview and PDF rendering.
export function getCoverPageFields(values: MndaFormValues): [string, string][] {
  const mndaTermDisplay =
    values.mndaTermType === 'years'
      ? `Expires ${getMndaTermText(values)}`
      : 'Continues until terminated in accordance with the terms of the MNDA';

  const confTermDisplay =
    values.confidentialityTermType === 'perpetuity'
      ? 'In perpetuity'
      : getConfidentialityTermText(values);
  ...
  return [
    ...
    ...(values.modifications.trim()
      ? ([['MNDA Modifications', values.modifications]] as [string, string][])
      : []),
  ];
}
...
// Form values are HTML-escaped before injection; bold and links are converted from markdown.
export function getStandardTermsHtml(values: MndaFormValues): string {
  const replacements: Record<string, string> = {
    Purpose: escapeHtml(values.purpose),
    'Effective Date': escapeHtml(formatDate(values.effectiveDate)),
    'MNDA Term': escapeHtml(getMndaTermText(values)),
    'Term of Confidentiality': escapeHtml(getConfidentialityTermText(values)),
    'Governing Law': escapeHtml(values.governingLaw),
    Jurisdiction: escapeHtml(values.jurisdiction),
  };

  const withBoldAndLinks = STANDARD_TERMS_TEMPLATE
  ...
  // Replace coverpage_link spans with highlighted field values (already escaped above)
  const withValues = withBoldAndLinks.replace(
    /<span class="coverpage_link">([^<]+)<\/span>/g,
    (_, fieldName) =>
      `<span class="bg-yellow-100 text-yellow-900 px-0.5 rounded font-medium">${
        replacements[fieldName] ?? escapeHtml(fieldName)
      }</span>`
  );
...
// Returns plain text with markers replaced — used by the PDF renderer.
export function getStandardTermsPlainText(values: MndaFormValues): string {
  const replacements: Record<string, string> = {
    Purpose: values.purpose,
    'Effective Date': formatDate(values.effectiveDate),
  ...
  return STANDARD_TERMS_TEMPLATE
    .replace(
      /<span class="coverpage_link">([^<]+)<\/span>/g,
      (_, fieldName) => replacements[fieldName] ?? fieldName
    )
    .replace(/\*\*([^*]+)\*\*/g, '$1')
    .replace(/\[([^\]]+)\]\([^)]+\)/g, '$1');
}
